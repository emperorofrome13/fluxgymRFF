...
import math
import random
import time # Added for timing iterations
...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    def __init__(self):
        ...
        self.last_iteration_time = None # Added to store the time of the last iteration
    ...
    def get_noise_pred_and_target(
        ...
        current_time = time.time() # Get current time at the start of the iteration processing
        if self.last_iteration_time is not None:
            iteration_duration = current_time - self.last_iteration_time
            logger.info(f"Time for previous iteration: {iteration_duration:.2f} seconds.")
        self.last_iteration_time = current_time # Update last_iteration_time for the next calculation
        ...
        logger.info(f"Iteration: Processed batch (size: {bsz}) for noise/target generation.") # Existing log message
